...
int function4(formats_t *formats, int count, int a, long long nb)
{
    if (formats->precision != 0 && formats->width == 0 && formats->flag1 != 8
        && formats->flag2 != 8 && formats->flag3 != 8 && formats->flag1 != 4
        && formats->flag2 != 4 && formats->flag3 != 4) {
...
int function5(formats_t *formats, int count, int a, long long nb)
{
    if (nb == 0) {
        my_putchar('0');
        count++;
    ...
    count += function4(formats, count, a, nb);
...
int specifier_x(va_list arguments_list, formats_t *formats)
{
    ...
    count = function5(formats, count, a, nb);
    return count;
}
